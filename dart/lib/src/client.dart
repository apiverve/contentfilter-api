...
class ContentfilterValidationException implements Exception {
  final List<String> errors;

  ContentfilterValidationException(this.errors);

  @override
  String toString() => 'ContentfilterValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/contentfilter?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [url] (required) - The URL or domain to check [format: url]
class ContentfilterClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'url': ValidationRule(type: 'string', required: true, format: 'url'),
  };

  ContentfilterClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/contentfilter',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ContentfilterValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ContentfilterValidationException(errors);
    }
  }

  /// Execute a request to the Content Filter API.
  ///
  /// Parameters are validated before sending the request.
  Future<ContentfilterResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ContentfilterException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ContentfilterResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ContentfilterException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ContentfilterException('Resource not found');
      } else {
        throw ContentfilterException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ContentfilterException) rethrow;
      throw ContentfilterException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Content Filter API client.
class ContentfilterException implements Exception {
  final String message;

  ContentfilterException(this.message);

  @override
  String toString() => 'ContentfilterException: $message';
}
